...
(define (new-inv p)
  (build-list 6 (lambda (i) (make-piece (add1 (floor (/ i 2))) p))))
  ...
  (local
    {(define ncols (tile-grid-ncols grid))
     (define render  ;; pick a render function for pieces based on the xray mode
       (if xray?
           (lambda (ps) (foldl
                    (lambda (p img) (overlay (render-piece-with-border 2 p) img))
                    empty-image ps))
           (lambda (ps) (render-piece (first ps)))))
...
(define (all-pieces? ps)
  (local
    {(define (count-piece p ps)
       (foldl (lambda (q n) (if (piece=? p q) (add1 n) n)) 0 ps))}
       ...
       (if (<= 0 inv-sq)
           ...
           (local
             {(define p 
              ...
              (define new-inv 
                (map-by-player (lambda (pinv) (lift-top-piece pinv inv-sq))
                               player 
                               inv))}
             (make-world (apply-intro-lift p g) 
                         ...
                         new-inv 
                         log
                         (make-drag p x y (lambda (sq) (make-intro p sq)))
                         wrld))
           ;; check for a shift
           (local
                 ...
                 (local
                   ...
                   (world-check-victory
                    ...
                    (pair-snd (apply-shft-lift brd-sq g))
                     ...
                     inv
                     log
                     (make-drag p x y (lambda (dst) (make-shft brd-sq dst)))
                     wrld)))
...
;; intro-moves : piece game (listof move) num -> (listof move)
;; generates a list of the possible intros for a piece
(define (intro-moves piece game list n)
  (cond [(= n 9) list]
        [(move-legal? (make-intro piece n) game)
         (intro-moves piece game 
                      (cons (make-intro piece n) list) (+ n 1))]
        [else (intro-moves piece game list (+ n 1))]))
(check-expect (intro-moves (make-piece 1 'blue) testgame1 empty 0)
              (list
               (make-intro (make-piece 1 'blue) 8)
               (make-intro (make-piece 1 'blue) 7)
               (make-intro (make-piece 1 'blue) 6)
               (make-intro (make-piece 1 'blue) 4)
               (make-intro (make-piece 1 'blue) 3)
               (make-intro (make-piece 1 'blue) 1)
               (make-intro (make-piece 1 'blue) 0)))

;; shft-moves : num game (listof move) num -> (listof move)
;; generates a list of the possible shifts for a piece
(define (shft-moves src game list n)
  (cond [(= n 9) list]
        [(move-legal? (make-shft src n) game)
         (shft-moves src game 
                     (cons (make-shft src n) list) (+ n 1))]
        [else (shft-moves src game list (+ n 1))]))
(check-expect (shft-moves 2 testgame1 empty 0)
              (list (make-shft 2 8) (make-shft 2 7) (make-shft 2 6) 
                    (make-shft 2 4) (make-shft 2 3) (make-shft 2 1) 
                    (make-shft 2 0)))

;; in-inv : piece inv -> bool
;; checks if a piece is in an inv
(define (in-inv p inv)
  (local {;; in-list : piece (listof pieces) -> bool
          ;; checks if a piece is in a list of pieces
          (define (in-list p listp)
            (cond [(empty? listp) #f]
                  [(equal? p (first listp)) #t]
                  [else (in-list p (rest listp))]))}
    (if (symbol=? (piece-player p) 'blue)
        (in-list p (pair-snd inv))
        (in-list p (pair-fst inv)))))
(check-expect (in-inv (make-piece 1 'blue) (game-inv testgame1)) #t)
(check-expect (in-inv (make-piece 1 'orange) (game-inv testgame1)) #t)

;; possible-moves : game -> (listof move)
;; return all the next moves (according to whose turn it is)
(define (possible-moves g)
  (local {(define all-intros
            (append (if (in-inv (make-piece 1 (game-next g))
                                (game-inv g))
                        (intro-moves (make-piece 1 (game-next g))
                                     g empty 0)
                        empty)
                    (if (in-inv (make-piece 2 (game-next g))
                                (game-inv g))
                        (intro-moves (make-piece 2 (game-next g))
                                     g empty 0)
                        empty)
                    (if (in-inv (make-piece 3 (game-next g))
                                (game-inv g))
                        (intro-moves (make-piece 3 (game-next g))
                                     g empty 0)
                        empty)))
          (define all-shifts
            (append (shft-moves 0 g empty 0)
                    (shft-moves 1 g empty 0)
                    (shft-moves 2 g empty 0)
                    (shft-moves 3 g empty 0)
                    (shft-moves 4 g empty 0)
                    (shft-moves 5 g empty 0)
                    (shft-moves 6 g empty 0)
                    (shft-moves 7 g empty 0)
                    (shft-moves 8 g empty 0)))}
    (append all-intros all-shifts)))
(check-expect (possible-moves testgame1)
              (list
               (make-intro (make-piece 1 'blue) 8)
               (make-intro (make-piece 1 'blue) 7)
               (make-intro (make-piece 1 'blue) 6)
               ...
               (make-intro (make-piece 2 'blue) 3)
               (make-intro (make-piece 2 'blue) 1)
               (make-intro (make-piece 2 'blue) 0)
...
(define (random-move game)
  (cond [(empty? (possible-moves game)) #f]
        [else (list-ref (possible-moves game) 
                        (random (length (possible-moves game))))]))
(random-move testgame1)

;; check-done : world -> bool
;; checks if a world is done
(define (check-done wrld)
  (match wrld
    [(world _ _ _ _ 'quit _) true]
    [else false]))

;; key-fn : world key -> world
;; handles keystrokes
(define (key-fn wrld k)
  (match wrld
    [(world g xray? inv log st cp)
     (cond [(drag? st) wrld]
           [else (cond [(world-victory? wrld)
                        (cond [(key=? k "y")
                               (new-world (game-next g))]
                              [(key=? k "n") 
                               (make-world g xray? inv log 'quit cp)]
                              [else #t])]
                       [else (cond [(key=? k "q")
                                    (make-world g xray? inv log 'quit cp)]
                                   [(key=? k "r")
                                    (new-world (game-next g))]
                                   [(key=? k "x")
                                    (make-world g (not xray?) inv log st cp)]
                                   [else wrld])])])]))
     
;; run-game : player (game -> move) -> world
;; given a first player and a moveo-choosing function,
...
(define (run-game first-player pick-move)
  (big-bang (new-world first-player)
            ...
            [stop-when check-done]))
(run-game 'blue random-move)