...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname proj3-seed) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
...
;;;;;;;;;;;;;;;;;;;; PROJECT 3 CODE ;;;;;;;;;;;;;;;;;;;;
(define testgame1 (make-game 'blue (make-pair
                                    ...
                                    (list (make-piece 1 'blue)
                                          ...
                                          (make-piece 2 'blue)
                                          (make-piece 3 'blue)))
                             (list empty empty (list (make-piece 3 'blue) (make-piece 2 'orange))
                                   empty empty (list (make-piece 3 'orange) (make-piece 2 'blue))
                                   empty empty empty)))

(define testgame2 (make-game 'orange (make-pair
                                      (list (make-piece 1 'orange)
                                            (make-piece 1 'orange)
                                            (make-piece 2 'orange)
                                            (make-piece 3 'orange))
                                      (list (make-piece 1 'blue)
                                            ...
                                            (make-piece 2 'blue)
                                            (make-piece 3 'blue)))
                             (list empty empty (list (make-piece 2 'orange))
                                   (list (make-piece 3 'blue)) 
                                   empty 
                                   (list (make-piece 3 'orange) (make-piece 2 'blue))
                                   empty empty empty)))

(define testgame3 (make-game 'blue (make-pair 
                                    (list (make-piece 1 'orange)
                                          (make-piece 1 'orange)
                                          (make-piece 2 'orange)
                                          (make-piece 2 'orange)
                             ...
                             (list (list (make-piece 1 'blue))
                                   ...
                                   (list (make-piece 3 'orange)) empty empty)))

(define testgame4 (make-game 'blue (make-pair 
                                    (list (make-piece 1 'orange)
                                          (make-piece 1 'orange)
                                          (make-piece 2 'orange)
                                          (make-piece 2 'orange)
...
(define testgame5 (make-game 'orange (make-pair
                                    (list (make-piece 1 'orange)
                                          (make-piece 1 'orange)
                                          (make-piece 2 'orange)
                                          (make-piece 3 'orange))
...
;; that the player to move actually possesses the piece in question
;; (in the case of an intro), and that the destination square is
;; available (per the previous function), and check that the player
;; is next.
;;
(define (move-legal? mv g)
  (match* (mv g)
    [((intro p sq) (game next inv brd))
     (and (symbol=? (piece-player p) next)
          (if (symbol=? next 'blue)
              (ormap (λ (q) (piece=? p q)) (pair-snd inv))
              (ormap (λ (q) (piece=? p q)) (pair-fst inv)))
          (square-available? p sq brd))]
    [((shft src dst) (game next inv brd))
     (local
       {(define ps (pieces-at brd src))}
       (and (cons? ps)
            (symbol=? (piece-player (first ps)) next)
            (square-available? (first ps) dst brd)))]))

;; apply-move : move game -> game
;; applies a move to a game and returns the subsequent state
(define (apply-move m g)
  (local {(define other (if (symbol=? (game-next g) 'blue)
                            'orange
                            'blue))
          (define board (game-board g))
          (define nextgame (cond [(and (intro? m)
                                       (move-legal? m g)) 
                                  (apply-drop (intro-piece m) 
                                              (intro-square m)
                                              (apply-intro-lift (intro-piece m) g))]
                                 [(and (shft? m)
                                       (move-legal? m g))
                                  (cond [(victory? other 
                                                   (pair-snd (apply-shft-lift (shft-src m) g)))
                                         (pair-snd (apply-shft-lift (shft-src m) g))]
                                        [else
                                         (apply-drop (pair-fst (apply-shft-lift (shft-src m) g))
                                                     (shft-dst m)
                                                     (pair-snd (apply-shft-lift (shft-src m) g)))])]
                                 [else (error "Invalid Move")]))}
    (make-game other (game-inv nextgame) (game-board nextgame))))
(check-error (apply-move (make-intro (make-piece 1 'orange) 0) testgame1))
(check-expect (apply-move (make-intro (make-piece 1 'blue) 0) testgame1)
                          (make-game
                           'orange
                           (make-pair
                            (list (make-piece 1 'orange) (make-piece 1 'orange) (make-piece 2 'orange) (make-piece 3 'orange))
                           ...
                           (list
                            ...
                            empty
                            empty)))
(check-expect (apply-move (make-shft 2 3) testgame1) testgame2)


;; listmax : (listof num) -> num
;; returns max of a list of nums
(define (listmax list)
  (local {(define (rec-max list i)
            (cond [(empty? list) i]
                  [(> (first list) i)
                   (rec-max (rest list) (first list))]
                  [else (rec-max (rest list) i)]))}
    (cond [(empty? list) (error "empty list")]
          [else (rec-max list (first list))])))
(check-expect (listmax (list 0 1 3 2)) 3)
(check-expect (listmax (list -1 0 1)) 1)
(check-expect (listmax (list 0 0 0 1 1 1)) 1)

;; listmin : (listof num) -> num
;; returns min of a list of nums
(define (listmin list)
  (local {(define (rec-min list i)
            (cond [(empty? list) i]
                  [(< (first list) i)
                   (rec-min (rest list) (first list))]
                  [else (rec-min (rest list) i)]))}
    (cond [(empty? list) (error "empty list")]
          [else (rec-min list (first list))])))
(check-expect (listmin (list 0 1 3 2)) 0)
(check-expect (listmin (list -1 0 1)) -1)
(check-expect (listmin (list 0 0 0 1 1 1)) 0)

;; minimax : nat game -> integer
;; given max ply and game, return score
(define (minimax ply g)
  (local {(define minmax (if (symbol=? (game-next g) 'orange)
                             listmax
                             listmin))}
    (cond [(victory? (game-next g) g) (score g)]
          [else (cond [(<= ply 1) (minmax (map (lambda (x) (score x))
                                               (map (lambda (x) (apply-move x g)) 
                                                    (possible-moves g))))]
                      [else (minmax (map (lambda (x) (minimax (sub1 ply) x))
                                         (map (lambda (x) (apply-move x g))
                                              (possible-moves g))))])])))
(check-expect (minimax 1 testgame1) 0)
(check-expect (minimax 1 testgame2) 1)
(check-expect (minimax 1 testgame3) -1)
(check-expect (minimax 2 testgame1) 0)

;; minimax-choose-move : nat game -> move
;; given max ply and game, choose best available move
(define (minimax-choose-move ply g)
  (local {(define gminmax (minimax ply g))
          ;; maxmoves : (listof moves) (listof moves) -> (listof moves)
          ;; finds the list of all moves with score = to minimax
          (define (maxmoves mvs maxes)
            (cond [(empty? mvs) maxes]
                  [(= (minimax (sub1 ply) (apply-move (first mvs) g))
                      gminmax)
                   (maxmoves (rest mvs) (cons (first mvs) maxes))]
                  [else (maxmoves (rest mvs) maxes)]))
          (define mvs (maxmoves (possible-moves g) empty))}
    (if (not (empty? mvs))
        (list-ref mvs (random (length mvs)))
        false)))
...
(define TT-size 17) ;; a prime number
(define TT (make-vector TT-size empty))

;; sq->hash : square game -> num
;; converts a square in a game to the corresponding part of the hash 
(define (sq->hash sq g)
  (local {(define pieces (list-ref (game-board g) sq))}
  (+ (if (member? (make-piece 3 'orange) pieces)
         32 0)
     (if (member? (make-piece 3 'blue) pieces)
         16 0)
     (if (member? (make-piece 2 'orange) pieces)
         8 0)
     (if (member? (make-piece 2 'blue) pieces)
         4 0)
     (if (member? (make-piece 1 'orange) pieces)
         2 0)
     (if (member? (make-piece 1 'blue) pieces)
         1 0))))
(check-expect (sq->hash 2 testgame1) 24)

;; game->hash : game -> num
;; converts a game to the corresponding game
(define (game->hash g)
  (+ (* (expt 2 49) (sq->hash 0 g))
     (* (expt 2 43) (sq->hash 1 g))
     (* (expt 2 37) (sq->hash 2 g))
     (* (expt 2 31) (sq->hash 3 g))
     (* (expt 2 25) (sq->hash 4 g))
     (* (expt 2 19) (sq->hash 5 g))
     (* (expt 2 13) (sq->hash 6 g))
     (* (expt 2 7) (sq->hash 7 g))
     (* (expt 2 1) (sq->hash 8 g))
     (if (symbol=? (game-next g) 'blue) 1 0)))
(check-expect (game->hash testgame1) 3298553757697) 
(check-expect (game->hash testgame2) 1133890240512) 
(check-expect (game->hash testgame3) 598134325772289) 
(check-expect (game->hash testgame4) 600333349027841) 
(check-expect (game->hash testgame5) 3298553757696) 

;; hash->sq : num num -> (listof pieces)
;; given a hash, computes the corresponding 
;; list of pieces in a square
(define (hash->sq hash sq)
  (local {;; dec->bin : num -> num
          ;; converts decimal to binary
          (define (dec->bin x)
            (string->number 
             (number->string hash 2) 10)) 
          (define localhash (string->number 
                             (substring (string-append (make-string (- 55 
                                                                       (string-length (number->string hash 2))) #\0)
                                                       (number->string hash 2))
                                        (* sq 6) (+ (* sq 6) 6)) 10))
          ;; build-sq : num symbol (listof pieces) num -> (listof pieces)
          ;; builds a square, with a local hash, a symbol for color
          ;; an empty list and a counter starting at zero
          (define (build-sq hash color pieces i)
            (cond [(= 0 hash) pieces]
                  [(odd? hash) (build-sq (/ (sub1 hash) 10) 
                                         (if (symbol=? color 'orange) 'blue 'orange)
                                         (cons (make-piece 
                                                (cond [(<= i 1) 1]
                                                      [(<= i 3) 2]
                                                      [(<= i 5) 3]
                                                      [else (error "hash error")]) 
                                                color)
                                               pieces)
                                         (add1 i))]
                  [else (build-sq (/ hash 10)
                                  (if (symbol=? color 'orange) 'blue 'orange)
                                  pieces
                                  (add1 i))]))}
    (build-sq localhash 'blue empty 0)))
(check-expect (hash->sq 3298553757697 2) 
              (list (make-piece 3 'blue) (make-piece 2 'orange)))
    
;; hash->game : num -> game
;; converts a hash to the corresponding game
(define (hash->game hash)
  (local {(define next (if (even? hash) 'orange 'blue))
          (define board (map (lambda (x) (hash->sq hash x)) 
                             (list 0 1 2 3 4 5 6 7 8)))
          ;; remove-inv : (listof pieces) inv -> inv
          ;; removes a list of pieces from an inv
          (define (remove-inv pieces inv)
            (cond [(empty? pieces) inv]
                  [(symbol=? (piece-player (first pieces))
                             'orange)
                   (remove-inv (rest pieces) 
                               (make-pair (remove (first pieces)
                                                  (pair-fst inv))
                                          (pair-snd inv)))]
                  [else (remove-inv (rest pieces)
                                    (make-pair (pair-fst inv)
                                               (remove (first pieces)
                                                       (pair-snd inv))))]))
          (define inv 
            (local {;; rec-remove : inv num -> inv
                    ;; removes pieces that are on the board from inv 
                    ;; recursively using a counter starting from 0
                    (define (rec-remove inv i)
                      (cond [(= i 9) inv]
                            [else (rec-remove 
                                   (remove-inv (pieces-at board i) inv) 
                                   (add1 i))]))}
              (rec-remove (make-pair (new-inv 'orange)
                                     (new-inv 'blue)) 0)))}
    (make-game next inv board)))
(check-expect (hash->game 3298553757697) testgame1)
(check-expect (hash->game 1133890240512) testgame2)
(check-expect (hash->game 598134325772289) testgame3)
(check-expect (hash->game 600333349027841) testgame4)
(check-expect (hash->game 3298553757696) testgame5)
...
(define (minimax-tt ply g)
  (local {(define minmax (if (symbol=? (game-next g) 'orange)
    ...
    (cond [(victory? (game-next g) g) (score g)]
          [(member? (game->hash g)
                    (map (lambda (x) (tt-entry-hash x))
                         (vector-ref TT (modulo (game->hash g)
                                                (vector-length TT)))))
           (local {(define entry (first (filter (lambda (x) (if (= (tt-entry-hash x)
                                                                   (game->hash g)) #t #f))
                                                (vector-ref TT (modulo (game->hash g)
                                                                       (vector-length TT))))))}
             (if (or (>= (tt-entry-depth entry) ply)
                     (not (= (tt-entry-score entry) 0)))
                 (tt-entry-score entry)
                 (begin 
                   (vector-set! TT
                              (modulo (game->hash g)
                                      (vector-length TT))
                              (cons (make-tt-entry (game->hash g)
                                                   (minimax ply g)
                                                   ply)
                                    (remove entry
                                            (vector-ref TT
                         ...
                         (vector-set! TT
                                      ...
                                      (cons (make-tt-entry (game->hash g)
                                            ...
                                            (vector-ref TT
                                                        (modulo (game->hash g)
                                                                (vector-length TT)))))
                         (minmax (map (lambda (x) (minimax (sub1 ply) x))
                                      (map (lambda (x) (apply-move x g))
                                           (possible-moves g)))))])])))
(check-expect (minimax-tt 1 testgame1) 0)
(check-expect (minimax-tt 1 testgame2) 1)
(check-expect (minimax-tt 1 testgame3) -1)
(check-expect (minimax-tt 2 testgame1) 0)
...
(define (minimax-tt-choose-move ply g)
  (local {(define gminmax (minimax-tt ply g))
          ;; maxmoves : (listof moves) (listof moves) -> (listof moves)
          ;; finds the list of all moves with score = to minimax
          (define (maxmoves mvs maxes)
            (cond [(empty? mvs) maxes]
                  [(= (minimax-tt (sub1 ply) (apply-move (first mvs) g))
                      gminmax)
                   (maxmoves (rest mvs) (cons (first mvs) maxes))]
                  [else (maxmoves (rest mvs) maxes)]))
          (define mvs (maxmoves (possible-moves g) empty))}
    (if (not (empty? mvs))
        (list-ref mvs (random (length mvs)))
        false)))
;; Couldn't think of any logical check-expects
...
(define (run-mm ply human-player)
  ((λ (x) true)
   (run-game human-player (lambda (x) (minimax-choose-move ply x)))))

;; run-mm-tt : nat player -> bool
;; runs game and returns true
(define (run-mm-tt ply human-player)
  ((λ (x) true)
   (run-game human-player (lambda (x) (minimax-tt-choose-move ply x)))))
